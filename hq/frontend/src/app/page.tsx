            ...
            {quickAccessCards.map((card) => {
              ...
              return (
                <Link key={card.href} href={card.href}>
                  <motion.div
                    whileHover={{ y: -4 }}
                    className="group h-full rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-200 hover:border-slate-300 hover:shadow-lg"
                  >
                    <div className={`inline-flex rounded-lg bg-gradient-to-br ${card.color} p-2 text-white mb-3`}>
                      <Icon className="h-4 w-4" />
                    </div>
                    <h3 className="text-sm font-bold text-slate-900">{card.title}</h3>
                    <p className="mt-1 text-xs text-slate-500 line-clamp-2">{card.desc}</p>
                    <p className="mt-3 text-xs font-medium text-slate-400 group-hover:text-slate-600">进入 →</p>
                  </motion.div>
                </Link>
              );
            })}
