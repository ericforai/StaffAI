...
- `api/` 只做协议转换、参数校验和调用编排，不承载领域逻辑
- `orchestration/` 只负责任务计划、状态推进和结果汇总，不直接处理 HTTP 细节
- `runtime/` 只负责执行器调用、超时、降级和执行结果，不直接决定业务路由
- `governance/` 只负责风险策略、审批与审计，不直接发起业务任务
- `memory/` 只负责上下文读取、检索和写回，不直接推进任务状态
- `observability/` 只负责事件、日志与追踪，不直接写业务主状态
- 新增文件默认不得继续向 500 行靠近；一旦跨越维护阈值就必须拆分
- 不允许跨 bounded context 直接改写对方主状态，只能通过显式接口调用
